...
//  ColGen_QuadIPPrice
//
//  Created by StephanP on 4/19/23.
//  Copyright © 2023 Stephan Patterson. All rights reserved.
//
//  CG on LP(general) copied from ColGenDelFS
//  Version for using the new pricing IP but still in Quadratic Form
//  For comparing performance of our linearized version vs. Gurobi's handling of the quadratic objective
...
//   modelprice->set(GRB_IntParam_Aggregate,0);
   ...
   // Here we set the linear objective costs
   ...
   modelprice->update();
   //In this version, there are no 4-indexed z-variables
   //The coefficients that were on the 4-indexed variables in the objective are now in the quadratic objective
   GRBQuadExpr obj = modelprice->getObjective();
   for (int i = 0; i < Pnum-1; ++i)
   {
      for (int j = i+1; j < Pnum; ++j)
      {
...
//               std::cout << vbases[i] <<std::endl;
   ...
   //      std::cout << currentobj << std::endl;
      ...
      fname2 <<"/Users/spatterson/ForXcode/PricingLP/Pricing_Test_Quad_"<<iter<<".lp";
      modelprice->write(fname2.str());
